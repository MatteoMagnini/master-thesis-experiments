...
local BASE_GROUND_COUNT = 8
local BASE_GROUND = collect(range(1, BASE_GROUND_COUNT))
local LIGHT_SENSORS_COUNT = 24
local LIGHT_SENSORS = collect(range(1, LIGHT_SENSORS_COUNT))
...
---@return number
function argos.get_wheels_distance()
    return robot.wheels.axis_length
end
...
---@param value_count_to_return number
---@return number[]
function argos.get_base_ground_values(value_count_to_return)
    local sensor_values =
        BASE_GROUND:mapValues(function (input_index) return robot.base_ground[input_index].value end)
    if(value_count_to_return == BASE_GROUND_COUNT) then return sensor_values:all() end

    return sensor_values:grouped(BASE_GROUND_COUNT / value_count_to_return)
            :mapValues(function (group) return collect(group):max() end):all()
end

---@param value_count_to_return number
---@return number[]
function argos.get_light_values(value_count_to_return)
    local sensor_values =
        LIGHT_SENSORS:mapValues(function (input_index) return robot.light[input_index].value end)
    if(value_count_to_return == LIGHT_SENSORS_COUNT) then return sensor_values:all() end

    return sensor_values:grouped(LIGHT_SENSORS_COUNT / value_count_to_return)
            :mapValues(function (group) return collect(group):max() end):all()
end
...
function argos.sensor_values_to_booleans(values, threshold, invert_values)
    return collect(values):mapValues(function (value)
