...
local function check_network(network)
  if network["boolean_functions"] == nil or network["connection_matrix"] == nil or network["state"] == nil or
          network["input"] == nil or network["output"] == nil or network["overridden_output"] == nil then
    error("missing one or more boolean network value in 'network' table")
  end
end
...
local function extract_input_output_nodes(options)
  ...
  --[[
    The following line assigns the input and outpu nodes in a random way.
    If you do not want this behaviour comment the line and remove the lines belowe it.
  --]]
  
  --local extracted_nodes = random_extract(input_output_nodes_count, 1, options["node_count"])
  
  local extracted_nodes = {}
  for i=1,input_output_nodes_count do extracted_nodes[i] = i end
...
function BooleanNetwork:load(network)
  check_network(network)
  self.boolean_functions = network["boolean_functions"]
  self.connection_matrix = network["connection_matrix"]
  self.node_states = network["state"]
  self.input_nodes = network["input"]
  self.output_nodes = network["output"]
  self.overridden_output_functions = network["overridden_output"]
  return(self)
end
